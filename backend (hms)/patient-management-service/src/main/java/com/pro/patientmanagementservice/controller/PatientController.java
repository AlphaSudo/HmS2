...
public class PatientController {
    ...
    @PreAuthorize("hasAnyRole('ADMIN', 'STAFF', 'DOCTOR') or @patientSecurity.isOwner(authentication, #patientId)")
    ...
    @PreAuthorize("hasAnyRole('ADMIN', 'STAFF', 'DOCTOR') or @patientSecurity.isOwnerByUserId(authentication, #userId)")
    ...
    @GetMapping("/doctor/{doctorAssigned}")
    @PreAuthorize("hasAnyRole('ADMIN', 'STAFF', 'DOCTOR')")
    public ResponseEntity<List<PatientProfileDTO>> getPatientsByDoctor(@PathVariable String doctorAssigned) {
        List<PatientProfileDTO> patients = patientService.getPatientsByDoctor(doctorAssigned);
        return ResponseEntity.ok(patients);
    }
    ...
    public ResponseEntity<PatientProfileDTO> updatePatientProfile(@PathVariable Long patientId, @Valid @RequestBody PatientProfileDTO patientProfileDTO) {
        PatientProfileDTO updatedPatient = patientService.updatePatientProfile(patientId, patientProfileDTO);
        return ResponseEntity.ok(updatedPatient);
    }

    @DeleteMapping("/{patientId}")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<PatientProfileDTO> deletePatient(@PathVariable Long patientId) {
        PatientProfileDTO deletedPatient = patientService.deletePatient(patientId);
