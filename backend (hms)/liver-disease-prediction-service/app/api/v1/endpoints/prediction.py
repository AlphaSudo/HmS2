...
from ....schemas.liver import LiverPatientData, LiverDiseasePredictionResponse, HealthResponse
...
async def service_info():
    ...
    return {
        "service": eureka_client.service_name,
        "version": "1.0.0",
        "description": "AI-powered liver disease risk prediction",
        "model_loaded": prediction_service.model_loaded,
        "eureka_server": eureka_client.eureka_server,
        "instance_id": eureka_client.instance_id
    }

@router.post("/predict", response_model=LiverDiseasePredictionResponse)
async def predict_liver_disease(patient_data: LiverPatientData, patient_id: Optional[str] = None):
    """
    Predict liver disease risk for a single patient.
    """
    try:
        ...
        return LiverDiseasePredictionResponse(
            liver_disease_risk=risk,
