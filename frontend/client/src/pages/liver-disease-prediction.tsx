...
// Based on backend/hms/liver-disease-prediction-service/app/schemas/liver.py
interface LiverPatientData {
  Age: number;
  Gender: string;
...
interface PredictionResponse {
  liver_disease_risk: number;
  probability: number;
  risk_level: string;
}

const initialFormData: LiverPatientData = {
    Age: 45,
    Gender: "Male",
    Total_Bilirubin: 0.9,
    ...
    Aspartate_Aminotransferase: 55,
    Total_Protiens: 7.0,
    Albumin: 3.8,
    Albumin_and_Globulin_Ratio: 1.1,
};

export default function LiverDiseasePredictionPage() {
  const { isRTL } = useLanguage();
  const { theme } = useTheme();
  const { t } = useTranslation();
  const [formData, setFormData] = useState<LiverPatientData>(initialFormData);
  ...
  const handlePrediction = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('http://localhost:8091/liver-disease-prediction/predict', {
        method: 'POST',
        headers: {
      ...
      toast({
        ...
        description: "Liver disease risk has been assessed.",
  ...
  return (
    <div className={cn("min-h-screen flex", theme === 'dark' ? 'bg-gradient-to-br from-[#0f0728] via-[#190a3e] to-[#0f0728] text-white' : 'bg-slate-50 text-slate-900')}>
      ...
      <div className={`flex-1 transition-all duration-300 ${isCollapsed ? (isRTL ? 'mr-16' : 'ml-16') : (isRTL ? 'mr-64' : 'ml-64')} p-8`}>
        <PatientHeader 
            title={t('liverPrediction.title', 'Liver Disease Prediction')} 
            subtitle={t('liverPrediction.subtitle', 'Assess liver disease risk using health metrics')} 
        ...
        <div className="flex-1 overflow-auto p-6">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            >
              ...
              >
                <h3 className="text-xl font-semibold mb-6 flex items-center gap-3">
                  ...
                  {t('liverPrediction.form.title', 'Patient Health Data')}
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {Object.keys(initialFormData).map((key) => (
                    <div key={key}>
                      <label className={cn("block text-sm font-medium mb-2", isLightMode ? "text-slate-700" : "text-gray-300")}>
                        {t(`liverPrediction.form.${key}`, key.replace(/_/g, ' '))}
                      </label>
                      {key === 'Gender' ? (
                        <select
                          name={key}
                          value={formData[key as keyof LiverPatientData]}
                          onChange={handleInputChange}
                          className={cn('w-full px-3 py-3 rounded-xl focus:ring-2 focus:border-transparent', theme === 'dark' ? 'bg-[#1a0a2e] border-white/20 text-white focus:ring-purple-500' : 'bg-slate-100 border border-slate-300 text-slate-900 focus:ring-indigo-500')}
                        >
                  ...
                  >
                    {isLoading ? (
                      <>
                        ...
                        {t('liverPrediction.buttons.assessing', 'Assessing...')}
                      </>
                    ) : (
                      <>
                        <Bot className="w-5 h-5" />
                        {t('liverPrediction.buttons.predict', 'Predict Risk')}
                      </>
                    )}
                  </button>
                </div>
            ...
            >
              ...
              >
                <h3 className="text-xl font-semibold mb-4 text-center">{t('liverPrediction.result.title', 'Prediction Result')}</h3>
                {isLoading && (
                  <div className="text-center">
                    <Activity className={cn("w-12 h-12 mx-auto animate-pulse", isLightMode ? "text-indigo-500" : "text-purple-400")} />
                    <p className={cn("mt-4", isLightMode ? "text-slate-600" : "text-gray-400")}>{t('liverPrediction.result.loading', 'Analyzing data...')}</p>
                  </div>
                )}
                {!isLoading && !prediction && (
                  <div className="text-center">
                    <Heart className={cn("w-12 h-12 mx-auto", isLightMode ? "text-slate-400" : "text-gray-500")} />
                    <p className={cn("mt-4", isLightMode ? "text-slate-600" : "text-gray-400")}>{t('liverPrediction.result.pending', 'Results will be displayed here.')}</p>
                  </div>
                )}
                {prediction && (
                  <motion.div 
                  ...
                  >
                    ...
                    <p className="mt-2 text-lg font-medium">{t('liverPrediction.result.riskOf', 'Risk of Liver Disease')}</p>
                    <div className={cn(
                    ...
                    <p className={cn("mt-4 text-sm", isLightMode ? "text-slate-600" : "text-gray-400")}>
                      {prediction.liver_disease_risk === 1 
                        ? t('liverPrediction.result.hasDisease', 'The model predicts the presence of liver disease.')
                        : t('liverPrediction.result.noDisease', 'The model does not predict liver disease.')}
                    </p>
                  </motion.div>
                )}
              </div>
